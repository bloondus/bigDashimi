...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Restaurant/Tisch-Suche initialisieren
    ...
    if (form) {
        form.onsubmit = async function(e) {
            ...
            const resultsEl = document.getElementById('restaurant-search-results');
            resultsEl.innerHTML = '<div class="loading">Suche läuft...</div>';
            try {
                let data = localStorage.getItem('restaurants-data');
                if (!data) {
                    const res = await fetch('assets/data/restaurants.json');
                    data = await res.json();
                    localStorage.setItem('restaurants-data', JSON.stringify(data));
                } else {
                    data = JSON.parse(data);
                }
                // Filter nach Adresse und Zeit
                const filtered = data.filter(r =>
                    r.address.toLowerCase().includes(address.toLowerCase()) &&
                    r.tables.some(t => t.time === time && t.available)
                );
                if (filtered.length === 0) {
                    resultsEl.innerHTML = '<div class="error">Keine passenden Restaurants oder freien Tische gefunden.</div>';
                } else {
                    resultsEl.innerHTML = filtered.map(r =>
                        `<div class='restaurant-result'><strong>${r.name}</strong><br>${r.address}<br>Frei um: ` +
                        r.tables.filter(t => t.time === time && t.available).map(t => t.time).join(', ') +
                        '</div>'
                    ).join('');
                }
            } catch (err) {
                resultsEl.innerHTML = '<div class="error">Fehler bei der Suche.</div>';
            }
        };
    }
});